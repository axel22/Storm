...
abstract class BasicModel(val size: (Int, Int)) extends Model with ModelAdapter {
  ...
  def characterAt(pos: (Int, Int)): Option[Character] = charactermap.at(pos)
  def characterPos(id: Long): Option[(Int, Int)] = charactermap.posWithId(id)
  def characters = charactermap.characters
  def character(id: Long) = charactermap.withId(id)
